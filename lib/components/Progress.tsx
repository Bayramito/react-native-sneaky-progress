...
const Progress: React.FC<ProgressProps> = ({
  ...
  const font = useFont(
require(fontFamily || "../assets/Gilroy-Regular.ttf"),
    FONT_SIZE
  );


  Array.from({ length: NUMBER_OF_CURVES }).forEach((_, i) => {
    if (i === 0) path.moveTo(HORIZONTAL_SHIFT, LINE_START_Y);
    const isOdd = i % 2 === 0;
    path.lineTo(
      HORIZONTAL_SHIFT + CURVE_GAP_X * i,
      isOdd ? LINE_HEIGHT : LINE_START_Y
    );
    path.rArcTo(RADIUS, RADIUS, 0, false, isOdd, CURVE_GAP_X, RADIUS);
    if (i === NUMBER_OF_CURVES - 1) {
      path.lineTo(
        HORIZONTAL_SHIFT + CURVE_GAP_X * (i + 1),
        isOdd ? LINE_START_Y : LINE_HEIGHT
      );
    }
  });
  const src = path.computeTightBounds();
  const centered = fitRect(
    src,
    rect(
      HORIZONTAL_SHIFT,
      RADIUS / 2,
      width - HORIZONTAL_SHIFT * 2,
      LINE_HEIGHT + CURVE_GAP_X + NUMBER_OF_CURVES
    )
  );
  ...
  const progress   = useDerivedValue(
    () =>
      withTiming(calculatedProgress / 100, { duration: 300 }, (isFinished) => {
        if (isFinished) {
          if (onStep) runOnJS(onStep)(step);
        }
      }),
    [step]
  );
    ...
    <View>
      {background && (
        <Background
          ...
          backgroundProps={{
            width: width,
            height: src.height + LINE_HEIGHT,
            ...backgroundProps,
          }}
        />
      )}
      <Canvas
        style={{ width: width, height: src.height + LINE_HEIGHT }}
        onTouch={onTouch}
      >
        <Group>
          <Shadow color={BACKGROUND_COLOR} dx={0} dy={0} blur={5} />
          <Path
            path={path}
            style="stroke"
        ...
        <Group>
          <Shadow color={WAY_SHADOW_COLOR} dx={0} dy={0} blur={15} />
          <Path
            path={path}
            style="stroke"
...
const StepCircle: React.FC<StepCircleProps> = ({
  ...
  border.addRRect(
    rrect(
      rect(
        point.x - (isLastStep ? STEP_SIZE * 1.1 : STEP_SIZE),
        point.y - (isLastStep ? STEP_SIZE * 1.1 : STEP_SIZE),
        isLastStep ? STEP_SIZE * 2.4 : STEP_SIZE * 2,
        isLastStep ? STEP_SIZE * 2.4 : STEP_SIZE * 2
      ),
      isLastStep ? STEP_SIZE * 2 : STEP_SIZE,
      isLastStep ? STEP_SIZE * 2 : STEP_SIZE
    )
  );
  return (
    <Group>
      <Circle
        cx={point.x}
        cy={point.y}
        r={isLastStep ? STEP_SIZE * 1.2 : STEP_SIZE}
        color={"white"}
      >
        {CURRENT_PROGRESS > index && (
          <Paint>
            <LinearGradient
              start={vec(point.x, point.y)}
              end={vec(point.x + STEP_SIZE, point.y + STEP_SIZE)}
              colors={CIRCLE_FILL_COLOR}
            />
          </Paint>
        )}
      ...
      <Text
        text={`${index + 1}`}
        x={point.x - (FONT_SIZE / 2) * (index >= 10 ? 0.75 : 0.75)}
        y={point.y + (FONT_SIZE / 2) * 0.7}
        font={font}
